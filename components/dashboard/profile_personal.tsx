...
function FieldInput({
  ...
  return (
    <View style={{ marginBottom: space.lg }}>
      ...
      />
      {note ? (
        <Text style={{ ...typography.bodyXS, color: colour.textSub, marginTop: space.xxs }}>
          {note}
        </Text>
      ) : null}
    </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={{
          ...
          <TouchableOpacity onPress={() => router.back()} style={{ marginBottom: space.sm }}>
            <Text style={{ ...typography.bodyS, color: 'rgba(255,255,255,0.8)' }}>‹ Settings</Text>
          </TouchableOpacity>
        ...
        <View style={{
          ...
          <View style={{ alignItems: 'center', paddingTop: space.xl, paddingBottom: space.lg }}>
            <View style={{
              ...
              <Text style={{ ...typography.h3, color: colour.onPrimary, fontWeight: '800' }}>
                {initials}
              </Text>
            </View>
            <Text style={{ ...typography.bodyS, color: colour.textSub, marginTop: space.xs }}>
              {user?.email}
            </Text>
          </View>
          ...
          <View style={{
            ...
            <Text style={{ ...typography.bodyM, fontWeight: '700', color: colour.text, marginBottom: space.md }}>
              Personal Details
            </Text>
            <View style={{ flexDirection: 'row', gap: space.md }}>
              <View style={{ flex: 1 }}>
                <FieldInput
                  label="First Name"
                  value={firstName}
                  onChangeText={(v) => wrap(setFullName)(`${v} ${lastName}`.trim())}
                />
              </View>
              <View style={{ flex: 1 }}>
                <FieldInput
                  label="Last Name"
                  value={lastName}
                  onChangeText={(v) => wrap(setFullName)(`${firstName} ${v}`.trim())}
                />
              </View>
            </View>
            <FieldInput
